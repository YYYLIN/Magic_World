...
namespace WWT
{
	namespace Management
	{
		...
		{
			Message _Message;
			WWT_Thread_SEM_Wait(m_WWT_SEM);
			WWT_Thread_Mutex_Lock(&m_MessageMutex);
			...
			WWT_Thread_Mutex_unLock(&m_MessageMutex);
		...
		SystemThread::~SystemThread()
		{
			...
			for (auto _auto = m_map_ThreadObject.begin(); _auto != m_map_ThreadObject.end();)
				UpdataStop(_auto);
			WWT_Thread_Mutex_unLock(&m_Mutex);

			m_S_pSystemThread = 0;
			WWT_Thread_Mutex_Destroy(&m_MutexPoolObject);
			WWT_Thread_Mutex_Destroy(&m_Mutex);
		}
		...
		{
			WWT_Thread_Mutex_Init(&m_Mutex);
			WWT_Thread_Mutex_Init(&m_MutexPoolObject);
			...
			if (_findTO != m_map_ThreadObject.end())
			{
				...
				WWT_Thread_Mutex_Init(&_findTO->second.m_MessageMutex);
			}
		...
		THREAD_OBJECT SystemThread::Create(const char* _name, UpdataCommon* _pUpdataCommon, ThreadTypeMode _ThreadTypeMode, ThreadMessageMode _ThreadMessageMode)
		{
			...
			if (_findTO != m_map_ThreadObject.end())
			{
				...
				m_set_ThreadObject.insert(&_findTO->second);
				WWT_Thread_Mutex_Init(&_findTO->second.m_MessageMutex);
				WWT_Thread_SEM_init(_findTO->second.m_WWT_SEM, NULL, 0, LONG_MAX, NULL, NULL, 0);
				if (WWT_Thread_Create(_findTO->second.m_Thread, NULL, ThreadFunction, (void*)(&_findTO->second)))
					WWT_ResumeThread(_findTO->second.m_Thread);
			}
			WWT_Thread_Mutex_unLock(&m_Mutex);
		...
		THREAD_POOL_OBJECT SystemThread::CreatePool(const char* _name, unsigned int _ThreadNumber)
		{
			...
			WWT_Thread_Mutex_Lock(&m_MutexPoolObject);
			...
			WWT_Thread_Mutex_unLock(&m_MutexPoolObject);
		...
		void SystemThread::ShutdownPool(THREAD_POOL_OBJECT _THREAD_POOL_OBJECT)
		{
			...
			WWT_Thread_Mutex_Lock(&_pThreadPoolObject->m_MessageMutex);
			for (auto _auto : _pThreadPoolObject->m_vec_ThreadObject)
			{
				Shutdown(_auto);
				WWT_Thread_SEM_Post(_pThreadPoolObject->m_WWT_SEM);
			}
			WWT_Thread_Mutex_unLock(&_pThreadPoolObject->m_MessageMutex);
			...
			for (auto _auto : _pThreadPoolObject->m_vec_ThreadObject)
			{
				...
				if (_IsHave)
				{
					WWT_Thread_Wait(_auto->m_Thread);
				}
			}

			WWT_Thread_SEM_destroy(_pThreadPoolObject->m_WWT_SEM);
			WWT_Thread_Mutex_Destroy(&_pThreadPoolObject->m_MessageMutex);
		...
		bool SystemThread::SendMessageTo(THREAD_OBJECT _THREAD_OBJECT, unsigned int _MessageType, long long _Message, Callback_Message _CallBack)
		{
			...
			WWT_Thread_Mutex_Lock(&_pThreadObject->m_MessageMutex);
			...
			if (_pThreadObject->m_ThreadMessageMode == THREAD_MESSAGE_WAIT)
				WWT_Thread_SEM_Post(_pThreadObject->m_WWT_SEM);
			WWT_Thread_Mutex_unLock(&_pThreadObject->m_MessageMutex);
		...
		void SystemThread::UpdataStop(MAP_SRTING_THREADOBJECT::iterator& _auto)
		{
			...
			{
				WWT_Thread_Wait(_auto->second.m_Thread);
				WWT_CloseHandle(_auto->second.m_Thread);
				WWT_MUTEX _Mutex = _auto->second.m_MessageMutex;
				WWT_SEM _SEM = _auto->second.m_WWT_SEM;

				m_set_ThreadObject.erase(&_auto->second);

				WWT_Thread_Mutex_Lock(&_Mutex);
				_auto = m_map_ThreadObject.erase(_auto);
				WWT_Thread_Mutex_unLock(&_Mutex);

				WWT_Thread_SEM_destroy(_SEM);
				WWT_Thread_Mutex_Destroy(&_Mutex);
			}
			else
				_auto++;
		}
		...
		int ThreadMessageHandle(ThreadObject* _pThreadObject)
		{
			...
			{
				if (_pThreadObject->m_ThreadMessageMode == THREAD_MESSAGE_WAIT)
					WWT_Thread_SEM_Wait(_pThreadObject->m_WWT_SEM);
				WWT_Thread_Mutex_Lock(&_pThreadObject->m_MessageMutex);
				...
				if (_Number)
				{
					...
					if (_pThreadObject->pUpdataCommon)
					{
						...
						{
							_pThreadMessage->MessageHandle(WWT_Message_Struct(_Message.m_MessageType, _Message.m_Message));
						}
					}
				}
