...
namespace Magic
{
	...
	float VectorIncludedAngle(glm::vec3 vector1, glm::vec3 vector2)
	{
	...
	float VectorIncludedAngle(glm::vec2 vector1, glm::vec2 vector2)
	{
		return (180 / (float)MAGIC_PI)*
			acos(
				glm::dot(vector1, vector2) /
	...
	float VectorIncludedRadian(glm::vec3 vector1, glm::vec3 vector2)
	{
		return acos(
			glm::dot(vector1, vector2) /
			(glm::length(vector1) * glm::length(vector2))
	...
	bool lat_to_H(double lat_d, double &H)
	{
	...
	bool H_to_lat(double H, double &lat_d)
	{
		const double PI = 3.141592653;
		double A2 = 0.0;
		double lat_r = 0.0;
	...
	float TwoLatLondistance(glm::vec2 _LatLon1, glm::vec2 _LatLon2)
	{
		return MtoNM(VectorIncludedRadian(SphereToVector(_LatLon1), SphereToVector(_LatLon2)) * EARTH_Radius);
	}

	float MtoNM(float _M)
	{
		return _M /= 1852.0f;
	}

	glm::vec3 SphereToDescartes(float _lon, float _lat, float _r)
	{
	...
	glm::vec3 SphereToVector(float _lon, float _lat)
	{
		_lon = glm::radians(_lon);
		_lat = glm::radians(_lat);
		float _sinlon = glm::sin(_lon);
	...
	glm::vec3 SphereToVector(glm::vec2 _lonlat)
	{
	...
	glm::vec2 VectorToSphere(glm::vec3 _vector)
	{
	...
	float TEMP_CtoF(float _data)
	{
		return _data * 1.8f + 32;
	}

	float TEMP_FtoC(float _data)
	{
		return (_data - 32) * 5 / 9;
	}

	float MEtoFT(float _data)
	{
		return _data * (float)FT_ME;
	}

	float FTtoME(float _data)
	{
